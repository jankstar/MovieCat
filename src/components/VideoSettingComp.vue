  ...
  methods: {
    ...
    VideoSettingBtn() {
      ...
      if (tracks) {
        ...
        this.VideoCapabilities = this.getCapabilities(tracks);
...
<template>
  <div>
    <q-card-section>
      ...
      <div v-if="SupportedConstraints.frameRate" class="text-body2">{{ $t("Frame_rate") }}: {{ Setting ?
        Setting.frameRate.toFixed(2) : "0" }}</div>
    </q-card-section>
    <q-separator />
  </div>
  ...
  <q-dialog v-model="VideoSettingDialog" no-backdrop-dismiss persistent class="tw-font-sans">
    <q-card style="width: 400px; height: 400px">
      <q-card-section>
        ...
        <q-select v-model="VideoSettingNew.enabled" :options="OnOffOptions" emit-value map-options
          :label="$t('VideoEnabled')" style="width: 45%" />
        <br />

        <div v-if="SupportedConstraints.width" class="text-body2">{{ $t("Resolution") }}: {{ VideoSettingNew ?
          `${VideoSettingNew.width}x${VideoSettingNew.height}` : "0x0" }}</div>
        <br v-if="SupportedConstraints.width" />

        <div v-if="SupportedConstraints.width" class="text-body2">{{ $t("Width") }}: {{ VideoSettingNew ?
          VideoSettingNew.width : "0" }}</div>
        <q-slider v-if="SupportedConstraints.width" v-model="VideoSettingNew.width" :min="VideoCapabilities.width.min"
          :max="VideoCapabilities.width.max" label style="width: 80%" />
        <div v-if="SupportedConstraints.height" class="text-body2">{{ $t("Height") }}: {{ VideoSettingNew ?
          VideoSettingNew.height : "0" }}</div>
        <q-slider v-if="SupportedConstraints.height" v-model="VideoSettingNew.height"
          :min="VideoCapabilities.height.min" :max="VideoCapabilities.height.max" label style="width: 80%" />
        <div class="text-body2">{{ $t("SampleRate") }}: {{ VideoSettingNew ? VideoSettingNew.frameRate.toFixed(0) : "0"
          }}</div>
        <q-slider v-if="SupportedConstraints.frameRate" v-model="VideoSettingNew.frameRate"
          :min="VideoCapabilities.frameRate.min" :max="VideoCapabilities.frameRate.max" label style="width: 80%" />
      </q-card-section>
      <q-separator />
